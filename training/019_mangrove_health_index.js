...
// Mask non mangrove
var maskedmangrove = composite.updateMask(mask);
Map.addLayer(maskedmangrove, {}, 'masked mangrove composite');

// Step 5. Compute the MHI using an expression.
var mhi = maskedmangrove.expression(
    '((102.12*((NIR - SWIR)/(NIR + SWIR)))-(4.64*(NIR/(GREEN-1)))+(178.15*((NIR-BLUE)/(NIR-RED)))+(159.53*((NIR-(2*RED)+BLUE)/(NIR+(2*RED)+BLUE)))-252.39)',{
      'NIR': maskedmangrove.select('B5_median'),
      'GREEN': maskedmangrove.select('B3_median'),
      'SWIR': maskedmangrove.select('B6_median'),
      'BLUE': maskedmangrove.select('B2_median'),
      'RED': maskedmangrove.select('B4_median'),
}).rename('mhi');

// Display MHI
Map.addLayer(mhi, {}, 'mhi');
